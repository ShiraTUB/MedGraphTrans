...
class WeightedHGTConv(MessagePassing):
    def __init__(self, in_dim, out_dim, num_types, num_relations, n_heads, dropout=0.2, use_norm=True, use_RTE=True, **kwargs):
        super(WeightedHGTConv, self).__init__(node_dim=0, aggr='add', **kwargs)
    ...
    def forward(self, node_inp, node_type, edge_index, edge_type, edge_time, edge_weight=None):
        # Added edge_weight to the forward method signature
        return self.propagate(edge_index, node_inp=node_inp, node_type=node_type,
                              edge_type=edge_type, edge_time=edge_time, edge_weight=edge_weight)

    def message(self, edge_index_i, node_inp_i, node_inp_j, node_type_i, node_type_j, edge_type, edge_time, edge_weight):
        '''
        ...
        for source_type in range(self.num_types):
            ...
            for target_type in range(self.num_types):
                ...
                for relation_type in range(self.num_relations):
                    ...
                    # Apply edge weights here. Ensure edge_weight is of shape [num_edges].
                    if edge_weight is not None:
                        # Expand edge_weight to match the shape of v_mat for broadcasting
                        edge_weight = edge_weight.view(-1, 1, 1)
                        v_mat = v_mat * edge_weight
...
class WeightedDenseHGTConv(MessagePassing):
    def __init__(self, in_dim, out_dim, num_types, num_relations, n_heads, dropout=0.2, use_norm=True, use_RTE=True, **kwargs):
        super(WeightedDenseHGTConv, self).__init__(node_dim=0, aggr='add', **kwargs)
...
class GeneralConv(nn.Module):
    def __init__(self, conv_name, in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm=True, use_RTE=True):
        ...
        if self.conv_name == 'hgt':
            self.base_conv = WeightedHGTConv(in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm, use_RTE)
        elif self.conv_name == 'dense_hgt':
            self.base_conv = WeightedDenseHGTConv(in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm, use_RTE)
