...
from src.old.random_link_split_qa import RandomLinkSplitQA


def random_link_split(data, neg_sampling_ratio):
    """Splits the data into train, test, and val data."""
    transform = RandomLinkSplitQA(
        num_val=0.1,
        num_test=0.1,
        is_undirected=True,
        disjoint_train_ratio=0.95,
        ...
        edge_types=("question", "question_correct_answer", "answer"),
        rev_edge_types=("answer", "rev_question_correct_answer", "question"),
    )
...
def build_link_neighbor_loaders(data, neg_sampling_ratio, num_neighbors, batch_size, val_neg_sampling_ratio=2.0):
    ...
    """
    split_data_dict = random_link_split(data, neg_sampling_ratio)

    split_loaders = {}
    edge_type = ("question", "question_correct_answer", "answer")

    for split_name, split_data in split_data_dict.items():
        edge_label = split_data[edge_type].pos_edge_label
        edge_label_index = (edge_type, split_data[edge_type].pos_edge_label_index)
