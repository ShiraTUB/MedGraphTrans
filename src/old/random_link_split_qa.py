...
@functional_transform('random_link_split_qa')
class RandomLinkSplitQA(BaseTransform):
    ...
    Args:
        ...
        rev_edge_types (Tuple[EdgeType] or List[Tuple[EdgeType]], optional):
            ...
            (default: :obj:`None`)
        neg_edge_types (Tuple[EdgeType] or List[Tuple[EdgeType]], optional):
        negative edges might be passed as an argument for negative sampling.
        (default: :tuple:`('question', 'question_wrong_answer', 'answer')`)
    """
    ...
    def __init__(
            ...
            neg_edge_types: Optional[Union[EdgeType, List[EdgeType]]] = ('question', 'question_wrong_answer', 'answer')
    ):
        ...
        self.neg_edge_types = neg_edge_types
    ...
    def _create_label(
        ...
        if hasattr(store, self.key):
            ...
            neg_edge_index = self.negative_sampler(data, store, index)
    ...
    def negative_sampler(self, data: HeteroData, store: EdgeStorage, index: Tensor):
        negative_examples = []
        source_node_indices = store.edge_index[:, index][0]
        for node_i in source_node_indices:
            negative_example_indices = torch.where(data[self.neg_edge_types].edge_index[0] == node_i)[0][:int(self.neg_sampling_ratio)]
            negative_examples.append(data[self.neg_edge_types].edge_index[:, negative_example_indices])
